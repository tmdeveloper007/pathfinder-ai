 ...
 * Uses only X-Forwarded-For with right-to-left parsing based on TRUSTED_PROXY_COUNT.
 * X-Real-IP is intentionally NOT used because it can be set by untrusted clients
 * and bypasses the trusted proxy chain validation.
 */
...
process.on("exit", () => {
  if (_defaultStore && typeof _defaultStore.close === "function") {
    _defaultStore.close();
  }
  if (_emergencyFallbackStore && typeof _emergencyFallbackStore.close === "function") {
    _emergencyFallbackStore.close();
  }
});
